...
//        $cat_str =     get_article_children($cat_id);
    ...
    if ($requirement != '')
    {
        $sql = 'SELECT article_id, title, author, add_time, file_url, open_type,description,content' .
...
//die($sql);
    ...
    if ($res)
    {
        while ($row = $GLOBALS['db']->fetchRow($res))
        {
            ...
            $arr[$article_id]['description']       = str_replace(array("\n", "\r","'"), '', $row['description']);
...
//        $count = $db->getOne("SELECT COUNT(*) FROM " . $ecs->table('article') . " WHERE " . get_article_children($cat_id) . " AND is_open = 1");
        $count = $db->getOne("SELECT COUNT(*) FROM " . $ecs->table('article') . " WHERE cat_id= " . $cat_id . " AND is_open = 1");
    }
    return $count;
}
