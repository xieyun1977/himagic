...
//    $smarty->assign('back_act', $back_act);
    /* 取出注册扩展字段 */
    $sql = 'SELECT content FROM ' . $ecs->table('article') . ' WHERE title like "注册协议"';
    $xieyi=$db->getOne($sql);
    $smarty->assign('xieyi',$xieyi);
    $smarty->display('user_reg.dwt');
...
elseif ($action == 'act_register')
{
    ...
    else
    {
        ...
        if (register($username, $password, $email, $other) !== false)
        {
            ...
            $smarty->display("user_reg_ok.dwt");
...
elseif ($action == 'get_password')
{
    ...
    if (isset($_GET['code']) && isset($_GET['uid'])) //从邮件处获得的act
    {
        ...
        $smarty->display('user_resetpwd.dwt');
//        $smarty->display('user_passport.dwt');
    }
    else
    {
        //显示用户名和email表单
        $smarty->display('user_getpwd.dwt');
//        $smarty->display('user_passport.dwt');
    }
}
...
elseif ($action == 'send_pwd_email')
{
    ...
    if ($user_info && $user_info['email'] == $email)
    {
        ...
        {
            $smarty->assign('ret','success');
            $smarty->assign('email',$email);
            $smarty->assign('info', $_LANG['send_success']);
...
//            show_message($_LANG['send_success'] . $email, $_LANG['back_home_lnk'], './', 'info');
        ...
        {
            //发送邮件出错
            $smarty->assign('ret','fail');
            $smarty->assign('info', $_LANG['fail_send_password']);
          //  show_message($_LANG['fail_send_password'], $_LANG['back_page_up'], './', 'info');
        }
//        $smarty->display('user_sendmail.dwt');
    }
    else
    {
        //用户名与邮件地址不匹配
        $smarty->assign('ret','fail');
        $smarty->assign('info', $_LANG['username_no_email']);
...
*/
   ...
   if($_GET["address_id"])
   {
       ...
       $provinces= get_regions(1, $consignee['country']);
       $citys    = get_regions(2, $consignee['province']);
       $districs= get_regions(3, $consignee['city']);
       $smarty->assign('provinces',$provinces);
       $smarty->assign('citys',$citys);
       $smarty->assign('districs',$districs);
   }
        /* 获取默认收货ID */
        $default_aid  = $db->getOne("SELECT address_id FROM " .$ecs->table('users'). " WHERE user_id='$user_id'");
    ...
    $smarty->assign('default_aid',          $default_aid);
