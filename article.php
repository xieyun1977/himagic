...
//echo $article_id;
$aname = get_artcat_name($article_id);
...
//公交信息
if($article_id=="gjxx"&&!$smarty->is_cached('gjxx.dwt', $cache_id))
{
    $smarty->display('gjxx.dwt', $cache_id);
    exit;
}
if (!$smarty->is_cached('article.dwt', $cache_id))
{
    ...
    {
        if(count(explode("?",$article['link']))>1)
            ecs_header("location:$article[link]&id=".$article_id."\n");
         else
            ecs_header("location:$article[link]?id=".$article_id."\n");
        exit;
    }
