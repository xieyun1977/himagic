...
if($_REQUEST['act']=='dp') {
    if (!$smarty->is_cached('buy_ticket.dwt', $cache_id)) {
        //获取门票商品分类
        $sql="select * from " . $ecs->table('goods') .
                    " WHERE is_on_sale = '1' and cat_id = '22'";
        $rows=$db->getAll($sql);
//        var_dump($rows);
        /* 获得商品的信息 */
        $smarty->assign('rows',$rows);
        $position = assign_ur_here('22','在线订票');
        $smarty->assign('page_title',$position['title']);
        assign_template();

        $smarty->display('buy_ticket.dwt', $cache_id);
    }
    exit;
}
else if (!$smarty->is_cached('goods.dwt', $cache_id))
{
