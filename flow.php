...
if ($_REQUEST['step'] == 'add_to_cart')
{
    ...
    $result = array('error' => 0, 'message' => '', 'content' => '', 'goods_id' => '','type'=>'');
    $json  = new JSON;
    ...
    else
    {
        ...
        if (addto_cart($goods->goods_id, $goods->number, $goods->spec, $goods->parent))
        {
            ...
            $result['content'] = insert_cart_info(11);
    ...
    $result['goods_id'] = $goods->goods_id;
    /**xieyun**/
    $result['type'] = $goods->type; //type   1：直接购买 2：订票
    $_SESSION['buydate']= $goods->buydate; //设置购买日期
    $_SESSION['number']= $goods->number; //设置购买日期
    die($json->encode($result));
}
...
elseif ($_REQUEST['step'] == 'select_payment')
{
    ...
    if (!check_consignee_info($consignee, $flow_type))
    {
        $result['error'] = $_LANG['flow_js']['address_not_null'];
    }
    else if (empty($cart_goods))
    {
        $result['error'] = $_LANG['no_goods_in_cart'];
    }
    else
...
//-- 门票购买下单
/*------------------------------------------------------ */
elseif($_REQUEST['step']=='tk_done')
{
    /*
         * 保存收货人信息
         */
    $consignee = array(
        'gid'     => empty($_POST['goods_id'])  ? '' :   compile_str(trim($_POST['goods_id'])),
        'username'     => empty($_POST['username'])  ? '' :   compile_str(trim($_POST['username'])),
        'idno'       => empty($_POST['idno'])    ? '' :   compile_str(trim($_POST['idno'])),
...
//die($consignee);
    ...
    {
        /* 如果用户已经登录，则保存收货人信息 */
        $consignee['uid'] = $_SESSION['user_id'];
        //获取已有收货信息
        $sql="select * from  " . $ecs->table('ticket_address') .  " WHERE gid = '" . $consignee['gid']  . "' "
            ." and uid='".$consignee['uid'] ."'";
        $rows=$db->getRow($sql);
        if(empty($rows['tid']))
        {
            /* 添加地址 */
            $res = $GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('ticket_address'), $consignee, 'INSERT');
//            $consignee['address_id'] = $GLOBALS['db']->insert_id();
        ...
        else
        {
            //修改地址
            /* 修改地址 */
            $res = $GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('ticket_address'), $consignee, 'UPDATE',
                'gid = ' . $consignee['gid']." AND uid= '". $consignee['uid']."'");
        }
...
//        'agency_id'       => get_agency_by_regions(array($consignee['country'], $consignee['province'], $consignee['city'], $consignee['district']))
    ...
    $new_order_id = $db->insert_id();
    $order['order_id'] = $new_order_id;
    $orderinfo=order_info($new_order_id);
    /* 插入订单商品 */
    $sql = "INSERT INTO " . $ecs->table('order_goods') . "( " .
        "order_id, goods_id, goods_name, goods_sn, product_id, goods_number, market_price, ".
        "goods_price, goods_attr, is_real, extension_code, parent_id, is_gift, goods_attr_id) ".
...
//    var_dump($order);
    ...
    $smarty->display('buy_ticket3.dwt');
    exit;
}
/*------------------------------------------------------ */
//-- 更新购物车
...
if(isset($_REQUEST['type'])&&$_REQUEST['type']=='2')  //订票第二步
{
    //获取已有收货信息
    $sql="select * from  " . $ecs->table('ticket_address') .  " WHERE gid = '" .$cart_goods['goods_list'][0]['goods_id'] . "' "
        ." and uid='".$_SESSION['user_id'] ."'";
//    echo $sql;
    $row=$db->getRow($sql);
    $smarty->assign('address',$row);
    $smarty->assign('buydate',$_SESSION['buydate']);
    $smarty->assign('number',$_SESSION['number']);
    $smarty->display('buy_ticket2.dwt');
}
else
    $smarty->display('goods_cart.dwt');
